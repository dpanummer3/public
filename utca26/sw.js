const SHELL_CACHE='utca-shell-v101';
const PHOTO_CACHE='utca-place-photo-v1';
const APP_SHELL=['/','/app.css?v=130','/app.js?v=138','/manifest.webmanifest?v=97','/onboarding-checkin.webp?v=97','/onboarding-options.webp?v=97'];
...
function cacheFirst(name,request){return caches.match(request).then(cached=>cached||fetch(request).then(response=>cachePut(name,request,response)).catch(()=>cached||new Response(null,{status:503})))}
